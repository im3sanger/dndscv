...
dndscv = function(mutations, gene_list = NULL, refdb = "hg19", substmodel = "submod_192r_3w", known_cancergenes = "hg19", covs = "hg19", max_muts_per_gene_per_sample = 3, max_coding_muts_per_sample = 3000, use_indel_sites = T, min_indels = 5, maxcovs = 20, constrain_wnon_wspl = T) {
    ...
    message("[3] Estimating the global rates...")
    ...
    par = fit_substmodel(N, L, substmodel) # Original substitution model
    ...
    par1 = fit_substmodel(N, L, s1) # Substitution model with 1 selection parameter
    s2 = gsub("wnon","wtru",gsub("wspl","wtru",substmodel))
    par2 = fit_substmodel(N, L, s2) # Substitution model with 1 selection parameter
    globaldnds = rbind(par, par1, par2)[c("wmis","wnon","wspl","wtru","wall"),]
    
    
    ## 4. Negative binomial regression to estimate the background model
    message("[4] Fitting the negative binomial regression model...")
    ...
    ## 5. Gene-level selection analyses
    message("[5] Selection tests at gene level...")
    
    ## dNdSloc: variable rate dN/dS model (gene mutation rate inferred from synonymous subs in the gene only)
    ...
    ## dNdScv: Negative binomial regression (with or without covariates) + local synonymous mutations
    ...
    dndscvout = list(globaldnds = globaldnds, sel_cv = sel_cv, sel_loc = sel_loc, annotmuts = annot, genemuts = genemuts, mle_submodel = mle_submodel, exclsamples = exclsamples, exclmuts = exclmuts)
