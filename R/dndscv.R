...
dndscv = function(mutations, gene_list = NULL, refdb = "hg19", sm = "192r_3w", kc = "cgc81", cv = "hg19", max_muts_per_gene_per_sample = 3, max_coding_muts_per_sample = 3000, use_indel_sites = T, min_indels = 5, maxcovs = 20, constrain_wnon_wspl = T, outp = 3, numcode = 1, outmats = F, mingenecovs = 500, onesided = F, dc = NULL, sitedcfile = NULL) {
  ...
  # [Input] Site-level duplex coverage [sitedcfile argument]
  #         This requires invoking buildcodon to create spatial vectors for each gene, and querying a bigwig input file with coverage information per site
  if (!is.null(sitedcfile)) {

    if (!is.null(dc)) {
      stop("dc and sitedcfile arguments cannot be used simultaneously")
    }
    ...
    for (j in 1:length(RefCDS)) {
      ...
      # CDS exonic positions
      dcvec = rep(RefCDS[[j]]$dcvec_cds, each=3) # Duplex coverage per site
  ...
  if (outp > 2) {
    ...
    if (nrow(indels) >= min_indels) {
      ...
      } else if (!is.null(sitedcfile)) {
        dcvec = sapply(RefCDS, function(x) sum(x$dcvec_cds) + sum(x$dcvec_spl))
        geneindels$cds_length = dcvec # We replace CDSlength for the total duplex coverage across the entire gene (including exonic and essential splice sites)
      }
