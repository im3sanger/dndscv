...
buildcodon = function(refcds, numcode = 1) {
    ...
    message("    Adding site-level information to RefCDS...")
    ...
    refcds[[1]]$trinuc_subsind = trinuc_subsind # Recording the mutation rate index vector in RefCDS
    ...
    for (j in 1:length(refcds)) {
        ...
        refcds[[j]]$codon_impact = as.integer(imp)
        refcds[[j]]$codon_rates = as.integer(unname(matrind))
